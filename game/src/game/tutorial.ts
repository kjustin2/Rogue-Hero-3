...
/** Five short lessons driven by the player's actual actions. */
export class Tutorial {
  ...
  private refreshObjective(): void {
    ...
    const lessons = [
      `Move with ${input.usingGamepad ? "the left stick" : input.labels("up", "left", "down", "right")}`,
      `Aim and press ${input.label("attack")} to ATTACK the husk`,
      `Press ${input.label("dodge")} to DASH — two charges. Dash through a hit for a PERFECT DODGE`,
      `Cast an ability with ${input.label("card1")}`,
      `Attacks build TEMPO — press ${input.label("crash")} to CRASH!`,
    ];
    const text = this.step >= 5 ? "TRAINING COMPLETE · The Rift awaits." : `TRAINING · ${lessons[this.step] ?? ""}`;
    if (text !== this.lastObjective) { this.lastObjective = text; this.hud.setObjective(text); }
  }
