...
/**
 * Combined color-grade effect (IDEAS-GRAPHICS #5 + #17 + #18 + #21) — the cheap ALU
 * work that reads as "graded" instead of "raw Three.js default", all in one pass:
...
export class GradeEffect extends Effect {
  ...
  /** Push the grade toward `color` by `amt` (0..1) — Tempo zone / death / victory. */
  setTint(color: Color, amt: number): void {
    (this.u("uTint").value as Color).copy(color);
    this.u("uTintAmt").value = amt;
  }

  /** Extra saturation (+victory) or drain (−death). */
